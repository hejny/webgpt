...
function handwriteText(options) {
  ...
  const handleWriteClick = () => {
    for (rr != null && window.cancelAnimationFrame(rr);
         canvasElement.lastChild;)
      canvasElement.removeChild(canvasElement.lastChild);
    if (((r = 0), (e = !1),
         options.style /* styleSelectElement.value */ == '-')) {
      const a = K($.g) / 64;
      var l = W(a * Random());
    } else
      l = options.style /*parseInt( styleSelectElement.value )*/;
    const o = options.text;

    (getElement('save-button').style.display = 'block'), (Z = o),
    ...
    var g = (r, e) => {
      const t =
          Math.round(options.speed /* parseFloat(speedSliderElement.value) */);
      for (let i = 0; i < t; i++) {
        const a = e[K(e) - 1];
        var [l, o, r] = L(a, r);
        if ((d += 1) > 40 * n || o > 0.5)
        ...
        e.push(l),
            (xi_c =
                 [
                   w[K(w) - 1][0] + v * l[0], w[K(w) - 1][1] - v * l[1], l[2]
                 ]),
            (xi_c = Y(xi_c)), w.push(xi_c);
      }
      e, (tr = w), S(w),
          (rr = window.requestAnimationFrame(() => { g(r, e); }));
    };
    g(h, p);
  };
  const q = (r, e, t) => {
    ...
    for (let d = 0; d < K(r); d++) {
      ...
      var f = [
        (f = [ t * (f = [ -v / i, n / i ])[0], t * f[1] ])[0] / u, f[1] / u
      ];
      let s = r[d][0] + 2 * f[0];
      let h = r[d][1] + 2 * f[1];
      a.push([ s, h ]);
    ...
    for (let r = 0; r < p; r++) {
      ...
      const x = j(m, T(M, 0.2));
      const C = I(g, T(y, 0.2));
      const A = `C ${sr(x[0])} ${sr(x[1])}, ${sr(C[0])} ${sr(C[1])}, ${
          sr(g[0])} ${sr(g[1])}`;
      c.push(A);
    }
    const k = c.join(' ');
    ...
    F.setAttribute('d', k), (F.style.stroke = options.color || 'black'),
        (F.style.fill = options.color || 'black'), canvasElement.appendChild(F);
  };
  var B = (r) => {
    const e = [];
    for (let o = 0; o < K(r); o++) {
...
*/
  ...
  fr = (() => {
    ...
    return setInterval(
        () => { (t.innerHTML = `Loading ${e[r % K(e)]}`), (r += 1); }, 200);
  })();

  // --------------------------------------- Press [Write!] button
  // ---------------------------------------

  fetch(ur).then((res) => { return res.arrayBuffer(); }).then((data) => {
    $ = ((data) => {
      let index = 0;
      const at = {};
      ...
      var init = (flightPhase) => {
        do {
          ...
          at[n] = content;
        } while (performance.now() - flightPhase < 16 &&
                 index < reader.byteLength);
