/*
 * File generated by Interface generator (dotup.dotup-vscode-interface-generator)
 * Date: 2023-05-04 14:05:34
 */
import { Promisable } from 'type-fest';
import { IVector, Vector } from 'xyzt';
import { Color } from '../color/Color';
import { WithTake } from '../take/interfaces/ITakeChain';

/**
 * Represents an image as a 2D array of pixels
 */
export interface IImage {
    /**
     * @@@
     */
    readonly size: Vector;
    /**
     * A getter for the width of the image
     */
    readonly width: number;
    /**
     * A getter for the height of the image
     */
    readonly height: number;
    /**
     * A method to get the color of a pixel at a given position
     * @param position The position of the pixel as an IVector object
     * @returns The color of the pixel as a Color object
     */
    getPixel(position: IVector): WithTake<Color>;
    /**
     * A method to set the color of a pixel at a given position
     * @param position The position of the pixel as an IVector object
     * @param color The color to set the pixel to as a Color object
     */
    setPixel(position: IVector, color: Color): void;
    /**
     * A method to crop the image
     *
     * @param point1 top left corner
     * @param point2 bottom right corner
     */
    crop(point1: IVector, point2: IVector): IImage;

    /**
     * @@@
     */
    map(callback: (color: Color) => Color): any /* <- TODO: How to refer here to extended Image */;

    /**
     * @@@
     */
    mapAsync(
        callback: (color: Color) => Promisable<Color>,
    ): Promise<any /* <- TODO: How to refer here to extended Image */>;
}
