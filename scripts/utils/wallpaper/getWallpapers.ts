...
import { COLORSTATS_CURRENT_VERSION, FONTS } from '../../../config';
...
async function findWallpapers(showWarnings: boolean): Promise<Array<IWallpaper>> {
    ...
    for (const metadataPath of wallpapersMetadataPaths) {
        ...
        if (colorStats.version !== COLORSTATS_CURRENT_VERSION) {
            if (showWarnings) {
                console.warn(
                    chalk.yellow(
                        ` ⏩  Skipping wallpaper ${id} Colors has different version "${colorStats.version}" (expected "${COLORSTATS_CURRENT_VERSION}")`,
                    ),
                );
            }
            continue;
